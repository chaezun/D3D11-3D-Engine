...
auto Terrain::GenerateVerticesAndIndices(Vector3 * positions) -> const bool
{
	...
	for (uint z = 0; z < height - 1; z++)
	{
		for (uint x = 0; x < width - 1; x++)
		{
			...
			index = bottom_left;
			indices_ptr[k] = index;
			vertices_ptr[index] = VertexTextureNormalTangentBlend(positions[index], Vector2(u_offset + 0.0f, v_offset + 1.0f));

			//Top Left
			index = top_left;
			indices_ptr[k + 1] = index;
			vertices_ptr[index] = VertexTextureNormalTangentBlend(positions[index], Vector2(u_offset + 0.0f, v_offset + 0.0f));

			//Bottom Right
			index = bottom_right;
			indices_ptr[k + 2] = index;
			vertices_ptr[index] = VertexTextureNormalTangentBlend(positions[index], Vector2(u_offset + 1.0f, v_offset + 1.0f));
			...
			vertices_ptr[index] = VertexTextureNormalTangentBlend(positions[index], Vector2(u_offset + 1.0f, v_offset + 0.0f));
